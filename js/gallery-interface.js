...
$(document).ready(function(){
  let randomPage = Math.floor(Math.random()*17057);
  console.log(randomPage);
  let promiseHarvy = new Promise(function(resolve, reject) {
    // let requestH = new XMLHttpRequest();
    // let urlH = `https://api.harvardartmuseums.org/image?apikey=${apiKey}&q=&page=${randomPage}&size=21`;
    // requestH.onload = function() {
    //   if (this.status === 200) {
    //     resolve(requestH.response);
    //   } else {
    //     reject(Error(requestH.statusText));
    //   }
    // };
    // requestH.open("GET", urlH, true);
    // requestH.send();
  });

  promiseHarvy.then(function(responseH) {
  //   let body = JSON.parse(responseH);
  //   for (let i = 0; i < body.records.length; i++) {
  //     if (body.records[i].baseimageurl !== null) {
  //       hArr.push({ "url" : body.records[i].baseimageurl, "frame" : ""});
  //     }
  //   }
  //   console.log(hArr);
  // }, function(error) {
